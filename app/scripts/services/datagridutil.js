 ...
 */
angular.module('ohanaApp').service('dataGridUtil', function() {
  'use strict';
  ...
  dataGridUtil.buildChaptersTableData = function(results) {
    ...
    for (var i = 0; i < resultsLen; i++) {
      ...
      for (var propName in results[i]) {
        if (
          results[i][propName] === null ||
          results[i][propName] === undefined ||
          results[i][propName] === true
        ) {
      ...
      for (var stateName in results[i]) {
        ...
        for (var chapterName in results[i][stateName]) {
          ...
          arr.googleMaps =
            ...
            results[i][stateName][chapterName].lat +
            ',' +
            results[i][stateName][chapterName].lng;
          arr.googleMaps_Link =
            "<a href='" +
            arr.googleMaps +
